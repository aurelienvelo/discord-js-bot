...
class ComponentsHandler {
    ...
    load = () => {
                ...
                try {
                    ...
                    if (module.__type__ === 3) {
                        if (!module.customId || !module.type || !module.run) {
                            this.client.logger.error('Unable to load the button/select/modal component ' + file);
                            continue;
                        }
                        ...
                        switch (module.type) {
                            ...
                            default: {
                                this.client.logger.error('Invalid component type (not: button, select, or modal): ' + file);
                                continue;
                            }
                        }

                        this.client.logger.info(`Loaded new component (type: ${module.type}) : ` + file);
                    } else if (module.__type__ === 4) {
                        if (!module.commandName || !module.run) {
                            this.client.logger.error('Unable to load the autocomplete component ' + file);
                            continue;
                        }

                        this.client.collection.components.autocomplete.set(module.commandName, module);

                        this.client.logger.info(`Loaded new component (type: autocomplete) : ` + file);
                    } else {
                        this.client.logger.error('Invalid component type ' + module.__type__ + ' from component file ' + file);
                    }
                } catch {
                    this.client.logger.error('Unable to load a component from the path: ' + 'src/component/' + directory + '/' + file);
                }
            }
        }
        ...
        this.client.logger.info(`Successfully loaded ${componentsCollection.autocomplete.size + componentsCollection.buttons.size + componentsCollection.selects.size + componentsCollection.buttons.size} components.`);
    }
