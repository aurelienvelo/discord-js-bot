                ...
                try {
                    ...
                    if (module.__type__ === 5) {
                        if (!module.event || !module.run) {
                            this.client.logger.error('Unable to load the event ' + file);
                            continue;
                        }
                        ...
                        this.client.logger.info(`Loaded new event: ` + file);

                        total++;
                    } else {
                        this.client.logger.error('Invalid event type ' + module.__type__ + ' from event file ' + file);
                    }
                } catch (err) {
                    this.client.logger.error('Unable to load a event from the path: ' + 'src/events/' + directory + '/' + file);
                }
            }
        }

        this.client.logger.info(`Successfully loaded ${total} events.`);
    }
}
