                ...
                try {
                    ...
                    if (module.__type__ === 2) {
                        if (!module.command || !module.run) {
                            this.client.logger.error('Unable to load the message command ' + file);
                            continue;
                        }
                        ...
                        this.client.logger.info('Loaded new message command: ' + file);
                    } else if (module.__type__ === 1) {
                        if (!module.command || !module.run) {
                            this.client.logger.error('Unable to load the application command ' + file);
                            continue;
                        }
                        ...
                        this.client.logger.info('Loaded new application command: ' + file);
                    } else {
                        this.client.logger.error('Invalid command type ' + module.__type__ + ' from command file ' + file);
                    }
                } catch {
                    this.client.logger.error('Unable to load a command from the path: ' + 'src/commands/' + directory + '/' + file);
                }
            }
        }

        this.client.logger.info(`Successfully loaded ${this.client.collection.application_commands.size} application commands and ${this.client.collection.message_commands.size} message commands.`);
    }
