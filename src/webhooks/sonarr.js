...
module.exports = {
    ...
    handleNotification: async (client, payload) => {
        const { instanceName, eventType, tv, remoteTv, release } = payload;

        // Trouver le canal Discord correspondant
        let channelId = config.webhookChannels.sonarr.channelMapping[instanceName]?.[eventType];
        if (!channelId) {
        ...
        try {
            ...
            const channelAdminId = config.webhookChannels.sonarr.admin;
            if (channelId) {
            ...
            const embed = {
                title: `Notification Sonarr ${eventType}`,
